%%
%Mitcheh and Fairweath ADI scheme  (MI scheme) solve 2D heat equation
...
ajx = ones(1,length(u(1,:))-3)*(-rx/2+1/12);
bjx = ones(1,length(u(1,:))-3)*(-rx/2+1/12);
djx = ones(1,length(u(1,:))-2)*(1-rx+1/6);
...
ajy = ones(1,length(u(1,:))-3)*(-ry/2+1/12);
bjy = ones(1,length(u(1,:))-3)*(-ry/2+1/12);
djy = ones(1,length(u(1,:))-2)*(1-ry+1/6);
...
for n = 1:length(t)
    ...
    for i = 2:length(x)-1
        uTx = u(i,2:end-1);
        cjx = uTx;
        cjx(1) = cjx(1)-bjx(1)*u(i,1);
        cjx(end) = cjx(end)-ajx(1)*u(i,end);
        %===============solving loop==========
    ...
    %y direction
    for j = 2:length(y)-1
        u(2:end-1,j) = (ry/2+1/12)*(u(2:end-1,j+1)+(1-ry-1/6)*u(2:end-1,j)+(ry/2+1/12)*u(2:end-1,j-1));
    end
%     
%     %step two
%     %x direction    
