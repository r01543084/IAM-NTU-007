%%
%Semi-classical ES-BGK equation in 2D
%Using WENO3 method 
...
cflx    = 0.1            ;% CFL number in x direction
cfly    = 0.1            ;% CFL number in y direction
...
mirco_v = repmat(mirco_v,[1 nx ny]);%將微觀速度向”速度空間”展開
weight  = repmat(weight ,[1 nx ny]);%將weight項也向”速度空間”展開，以利積分

%時間離散
dtx = dx*cflx/max(mirco_v(:,1));
dty = dy*cfly/max(mirco_v(:,1));
dt = min(dtx,dty);
time = 0:dt:tEnd;
...
% depend on x length
[z0,marco_u0,T0] = IC(nx,ny);

%將各巨觀量向”速度空間”展開
z       = repmat(z0      ,[nv 1 1]);%Fugacity
marco_u = repmat(marco_u0,[nv 1 1]);%Macroscopic Velocity 
T       = repmat(T0      ,[nv 1 1]);%Temperature
